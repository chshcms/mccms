...
<div class="layui-fluid">
    <div class="layui-card" style="padding: 20px;">
        ...
        <form class="layui-form" action="<?=links('generate','book_index')?>">
        ...
        <form class="layui-form" action="">
            ...
            <div class="layui-form-item">
                <label class="layui-form-label">按小说ID:</label>
                <div class="layui-input-inline">
...
layui.config({
    ...
    form.on('submit(home)', function (data) {
        $.post('<?=links('generate','book_index')?>/'+data.elem.dataset.type, {op:'index'}, function(res) {
            if(res.code == 1){
    ...
    form.on('submit(tpl)', function (data) {
        ...
        Admin.open('分类生成','<?=links('generate','mark')?>/'+data.elem.dataset.type+'?op=blist&id='+cid.join(','),600,400);
        return false;
    });
    //详情页
    form.on('submit(tplcid)', function (data) {
        var cid = formSelects.value('select','val');
        var day = $('#day').val();
        Admin.open('小说生成','<?=links('generate','mark')?>/'+data.elem.dataset.type+'?op=info&do=cid&id='+cid.join(',')+'&day='+day,600,400);
        return false;
    });
    form.on('submit(tplid)', function (data) {
        ...
        if(ksid == '' && jsid == ''){
            ...
            Admin.open('小说生成','<?=links('generate','mark')?>/'+data.elem.dataset.type+'?op=info&do=id&ksid='+ksid+'&jsid='+jsid,600,400);
        }
        return false;
    });
    //阅读页
    form.on('submit(tplcid2)', function (data) {
        var day = $('#day').val();
        Admin.open('章节生成','<?=links('generate','mark')?>/'+data.elem.dataset.type+'?op=read&do=cid&day='+day,600,400);
        return false;
    });
    form.on('submit(tplid2)', function (data) {
        ...
        if(ksid == '' && jsid == ''){
            ...
            Admin.open('章节生成','<?=links('generate','mark')?>/'+data.elem.dataset.type+'?op=read&do=id&ksid='+ksid+'&jsid='+jsid,600,400);
        }
        return false;
    });
});
