...
class Wxpay {
	...
	{
		//商户密钥
		$this->app_id = Pay_Wx_ID;
		$this->mch_id = Pay_Wx_User;
		$this->mch_key = Pay_Wx_Key;
		//同步地址
		$this->return_url = 'http://'.Web_Url.Web_Path.'index.php/api/pay/return_url';
		//异步地址
		$this->notify_url = 'http://'.Web_Url.Web_Path.'index.php/api/pay/notify_url/wxpay';
	}
    ...
    public function h5($dingdan,$total_fee,$body='vip'){
		...
		$arr['notify_url'] = $this->notify_url;
		$arr['trade_type'] = 'MWEB';
		$arr['scene_info'] = '{"h5_info": {"type":"Wap","wap_url": "'.$this->return_url.'","wap_name": "'.Web_Name.'"}}';
		$arr['sign'] = $this->getsign($arr);
		$url = 'https://api.mch.weixin.qq.com/pay/unifiedorder';
		$post_xml = $this->arrtoxml($arr);
		$xml = $this->geturl($url,$post_xml);
